<section class="max-w-2xl mx-auto mt-12 p-4 bg-white shadow-md border border-gray-200 rounded">
  <form enctype="multipart/form-data" method="POST" class="grid grid-cols-1 gap-6">
    <?php
    var_dump($this->resolve("partials/_csrf.php")); ?>
    ...
    <label class="block">
      ...
      <label class="block">
          ...
          <textarea  maxlength="2000" name="text" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"><?php echo escapeInjection($oldFormData['article_text'] ?? ''); ?></textarea>
          <?php if (isset($errors) && array_key_exists('description', $errors)) : ?>
              <div class="bg-gray-100 mt-2 p-2 text-red-500">
                  <?php echo escapeInjection($errors['description'][0]); ?>
              </div>
